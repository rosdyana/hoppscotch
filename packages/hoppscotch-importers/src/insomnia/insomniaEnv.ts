...
import { safeParseJSONOrYAML } from "../utils/yaml"
...
export const insomniaEnvImporter = (contents: string[]) => {
  ...
  /**
   * NOTE: @hoppscotch/data declares NonSecretEnvironmentVariable as
   * Extract<EnvironmentVariable, { secret: false }>, which resolves to `never`
   * because the union member types `secret` as boolean rather than a literal.
   * That makes NonSecretEnvironment impossible to construct, so describe the
   * variable shape locally. Runtime behaviour is unchanged.
   */
  type InsomniaEnvVariable = {
    key: string
    initialValue: string
    currentValue: string
    secret: false
  }
  type InsomniaEnvironment = Omit<NonSecretEnvironment, "variables"> & {
    variables: InsomniaEnvVariable[]
  }
  ...
  insomniaEnvs.forEach((insomniaEnv) => {
    ...
    if (parsedInsomniaEnv.success) {
      const environment: InsomniaEnvironment = {
        id: uniqueID(),
        v: EnvironmentSchemaVersion,
        name: parsedInsomniaEnv.data.name,
        // Object.entries widens the value to unknown; the Insomnia schema
        // guarantees a string here.
