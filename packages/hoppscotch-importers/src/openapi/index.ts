...
import { IMPORTER_INVALID_FILE_FORMAT } from "../errors"
...
export type OpenAPIDocProcessors = {
  validate: (docs: any) => Promise<OpenAPI.Document>
  dereference: (docs: any) => Promise<OpenAPI.Document>
}

/**
 * @param processors Override validation/dereferencing. hoppscotch-common
 * passes worker-backed versions so a large spec does not block the UI thread;
 * the default calls SwaggerParser directly, which is what the backend wants.
 */
export const hoppOpenAPIImporter = (
  fileContents: string[],
  processors: OpenAPIDocProcessors = {
    validate: validateDocs,
    dereference: dereferenceDocs,
  }
) =>
  pipe(
          ...
          async () => {
            ...
            for (const docObj of docArr) {
              try {
                ...
                try {
                  const validatedDoc = await processors.validate(docObj)
      ...
      pipe(
        TE.tryCatch(
          async () => {
            ...
            for (const docObj of docArr) {
              try {
                const validatedDoc = await processors.dereference(docObj)
                resultDoc.push(validatedDoc)
              } catch (_error) {
...
/**
 * Validate and dereference directly.
 *
 * hoppscotch-common runs these inside a web worker purely to keep the UI
 * responsive; there is no such concern here, and a worker would not exist in
 * Node anyway.
 */
const validateDocs = (docs: any): Promise<OpenAPI.Document> =>
  SwaggerParser.validate(docs) as Promise<OpenAPI.Document>

const dereferenceDocs = (docs: any): Promise<OpenAPI.Document> =>
  SwaggerParser.dereference(docs) as Promise<OpenAPI.Document>
