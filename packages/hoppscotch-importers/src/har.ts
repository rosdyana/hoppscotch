...
/**
 * The multipart variant of HoppRESTReqBody.
 *
 * @hoppscotch/data does not re-export this type from its index, so derive it
 * from the exported union rather than importing a name that does not exist.
 */
type HoppRESTReqBodyFormData = Extract<
  HoppRESTReqBody,
...
const convertPostDataToHoppBody = (
  ...
  // application/octet-stream carries a File, not a string, so an empty binary
  // body is null. (The original in hoppscotch-common returns "" here, which
  // does not satisfy the body schema - that file is outside its typecheck
  // scope, so the mismatch went unnoticed.)
